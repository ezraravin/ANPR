...
# %%
# LOAD THE IMAGE
img = cv2.imread('./test_images/17.jpg')
...
blob = cv2.dnn.blobFromImage(
    img, 1, (new_width, new_height), (123.68, 116.78, 103.94), True, False)
...
fin_boxes = non_max_suppression(
    np.array(rectangles), probs=confidence_score, overlapThresh=0.5)
...
for (x1, y1, x2, y2) in fin_boxes:
    ...
    blob = cv2.dnn.blobFromImage(
        segment_gray, scalefactor=1/127.5, size=(100, 32), mean=127.5)
    ...
    cv2.putText(img_copy, text.strip().upper(), (x1, y1-2),
...
for (x1, y1, x2, y2) in fin_boxes:
    ...
    ret, thresh = cv2.threshold(
        segment_gray, 0, 255, cv2.THRESH_BINARY+cv2.THRESH_OTSU)

    text = pytesseract.image_to_string(thresh, config=r"--psm 8")

    cv2.rectangle(img_copy, (x1, y1), (x2, y2), (0, 255, 0), 2)
    cv2.putText(img_copy, text.strip(), (x1, y1-2),
                cv2.FONT_HERSHEY_COMPLEX, 0.7, (0, 0, 255), 2)
    print(text.strip().upper())
...
while cap.isOpened():
    ...
    blob = cv2.dnn.blobFromImage(
        img, 1.0, (new_width, new_height), (123.68, 116.78, 103.94), True, False)
